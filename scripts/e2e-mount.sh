#!/usr/bin/env bash
# =============================================================================
# dsh-enhanced-workspace 挂载冒烟编排（CI + 本地）：
#
...
#   2. 校验 `dsh.profile.bundles` 已包含 dsh-enhanced-workspace；
#   3. 启动真实 `dsh web`（keyless，--port 0 取 OS 分配端口）；
#   4. 运行 tests/e2e 无头渲染 lane（Playwright Chromium）：断言
#      `[data-dsh-enhanced-workspace="browser"]` 遮蔽生效、侧栏挂载无崩溃。
#
# 用法：
...
#   TARBALL        插件 tarball；缺省仓库根最近的 dsh-enhanced-workspace-*.tgz
...
# dsh CLI 解析：PATH 上的 dsh 优先，否则 npx 拉官方包（市场安装同路径）
...
# tarball 解析（多个候选时取 mtime 最新，避免把冒烟挂到过期产物上）
if [ -z "$TARBALL" ]; then
  TARBALL="$(ls -t "$ROOT"/$NAME-*.tgz 2>/dev/null | head -1 || true)"
  COUNT="$(ls "$ROOT"/$NAME-*.tgz 2>/dev/null | wc -l | tr -d ' ')"
  [ "$COUNT" -le 1 ] || warn "发现 $COUNT 个 tarball，按 mtime 选用最新：$(basename "$TARBALL")（建议清理其余）"
fi
[ -n "$TARBALL" ] && [ -f "$TARBALL" ] || die "找不到 tarball（TARBALL 或 \$ROOT/$NAME-*.tgz）——先运行 pnpm build && pnpm pack"
...
# strict-dep-builds 拦截 node-pty/protobufjs 或拒绝 <24h 新版本）
...
# 步骤 3：校验挂载生效（dsh.profile.bundles 含 dsh-enhanced-workspace）
if ! node -e '
  const fs = require("fs");
  const p = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
  const bundles = p.dsh?.profile?.bundles ?? [];
  process.exit(bundles.includes("dsh-enhanced-workspace") ? 0 : 1);
' "$PROFILE_DIR/package.json"; then
  warn "dsh-enhanced-workspace 未出现在 dsh.profile.bundles 中——挂载未注册"
  cat "$PROFILE_DIR/package.json"
  exit 1
fi
say "挂载已注册：dsh.profile.bundles 包含 dsh-enhanced-workspace"
